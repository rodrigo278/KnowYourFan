...
st.set_page_config(page_title="Conheça Seu Fã - FURIA",
                   page_icon="🎮",
                   layout="wide",
...
with col1:
    st.image("https://upload.wikimedia.org/wikipedia/pt/f/f9/Furia_Esports_logo.png", width=80)
with col2:
    st.title("Conheça Seu Fã - FURIA")
    st.subheader(
        "Crie seu perfil de fã para desbloquear experiências exclusivas")
...
with steps_col1:
    st.markdown(
        f"**{'1. Dados Pessoais' if st.session_state.step != 1 else '→ 1. Dados Pessoais'}**"
    )
with steps_col2:
    st.markdown(
        f"**{'2. Interesses' if st.session_state.step != 2 else '→ 2. Interesses'}**"
    )
with steps_col3:
    st.markdown(
        f"**{'3. Verificação' if st.session_state.step != 3 else '→ 3. Verificação'}**"
    )
with steps_col4:
    st.markdown(
        f"**{'4. Redes Sociais' if st.session_state.step != 4 else '→ 4. Redes Sociais'}**"
    )
with steps_col5:
    st.markdown(
        f"**{'5. Painel' if st.session_state.step != 5 else '→ 5. Painel'}**")
...
if st.session_state.step == 1:
    ...
    with st.form("personal_info_form"):
        ...
        with col1:
            name = st.text_input(
                "Nome Completo",
                value=st.session_state.user_data['personal'].get('name', ''))
            email = st.text_input(
                "Email",
                value=st.session_state.user_data['personal'].get('email', ''))
            cpf = st.text_input(
                "CPF",
                value=st.session_state.user_data['personal'].get('cpf', ''))
            phone = st.text_input(
                "Telefone",
                value=st.session_state.user_data['personal'].get('phone', ''))

        with col2:
            address = st.text_input(
                "Endereço",
                value=st.session_state.user_data['personal'].get(
                    'address', ''))
            city = st.text_input(
                "Cidade",
                value=st.session_state.user_data['personal'].get('city', ''))
            state = st.text_input(
                "Estado",
                value=st.session_state.user_data['personal'].get('state', ''))
            birth_date = st.date_input(
                "Data de Nascimento",
                value=datetime.strptime(
                    st.session_state.user_data['personal'].get(
                        'birth_date',
                        datetime.today().strftime('%Y-%m-%d')), '%Y-%m-%d')
                if 'birth_date' in st.session_state.user_data['personal'] else
                None)
        ...
        if submitted:
            form_data = {
                ...
                state,
                'birth_date':
                birth_date.strftime('%Y-%m-%d') if birth_date else None
            }
            ...
            if empty_fields:
                st.error(
                    f"Por favor, preencha os seguintes campos obrigatórios: {', '.join(empty_fields)}"
                )
            else:
...
# Etapa 2: Interesses e Atividades
    ...
    with st.form("interests_form"):
        ...
        games_options = [
            "League of Legends", "Counter-Strike", "Valorant", "Dota 2",
            "Overwatch", "Fortnite", "Rainbow Six Siege", "Rocket League",
            "Outro"
        ]
        favorite_games = st.multiselect(
            "Selecione seus jogos favoritos",
            games_options,
            default=st.session_state.user_data['interests'].get(
                'favorite_games', []))

        # Inicialize other_games para corrigir o erro de "possibly unbound"
        other_games = ""
        if "Outro" in favorite_games:
            other_games = st.text_input(
                "Especifique outros jogos",
                value=st.session_state.user_data['interests'].get(
                    'other_games', ''))

        # Times favoritos
        st.subheader("Times Favoritos")
        teams_options = [
            "FURIA", "LOUD", "Team Liquid", "paiN Gaming", "Cloud9", "Fnatic",
            "G2 Esports", "T1", "FaZe Clan", "Outro"
        ]
        favorite_teams = st.multiselect(
            "Selecione seus times favoritos",
            teams_options,
            default=st.session_state.user_data['interests'].get(
                'favorite_teams', []))
        ...
        if "Outro" in favorite_teams:
            other_teams = st.text_input(
                "Especifique outros times",
                value=st.session_state.user_data['interests'].get(
                    'other_teams', ''))

        # Eventos frequentados
        st.subheader("Eventos Frequentados no Último Ano")
        attended_events = st.text_area(
            "Liste os eventos que você frequentou (um por linha)",
            value=st.session_state.user_data['interests'].get(
                'attended_events', ''))

        # Hábitos de jogo
        st.subheader("Hábitos de Jogo")
        hours_gaming = st.slider(
            "Horas jogando por semana", 0, 50,
            st.session_state.user_data['interests'].get('hours_gaming', 10))
        hours_watching = st.slider(
            "Horas assistindo esports por semana", 0, 30,
            st.session_state.user_data['interests'].get('hours_watching', 5))

        # Compras de produtos
        st.subheader("Compras de Produtos")
        merch_options = [
            "Camisetas de Times", "Acessórios de Times",
            "Equipamentos de Gaming", "Colecionáveis", "Nenhum"
        ]
        merchandise = st.multiselect(
...
# Etapa 3: Verificação de Documentos
    ...
    with col1:
        st.write(
            "Por favor, carregue seus documentos de identificação para verificação."
        )

        # Upload de Documento de Identidade
        st.subheader("Documento de Identidade")
        id_doc = st.file_uploader(
            "Carregue seu documento de identidade (frente)",
            type=["jpg", "jpeg", "png"])
        ...
        if id_doc:
            ...
            if st.button("Validar Documento"):
                with st.spinner("Processando documento..."):
                    ...
                    is_valid, validation_message = validate_document(
                        extracted_text, st.session_state.user_data['personal'])

                    if is_valid:
                        st.success(validation_message)
                        st.session_state.user_data['documents'][
                            'id_validated'] = True
                        st.session_state.user_data['documents'][
                            'id_validation_message'] = validation_message
                    else:
        ...
        secondary_doc = st.file_uploader(
            "Carregue outro documento para verificação adicional",
            type=["jpg", "jpeg", "png"])

        if secondary_doc:
            st.image(secondary_doc,
                     caption="Documento Secundário Carregado",
                     width=300)
    ...
    with col2:
        ...
        if st.session_state.user_data['documents'].get(
                'id_validated') or st.button("Pular Verificação"):
            next_step()
...
# Etapa 4: Integração com Redes Sociais
    ...
    with col1:
        with st.form("social_media_form"):
            ...
            twitter_username = st.text_input(
                ...
                value=st.session_state.user_data['social_media'].get(
                    'twitter_username', ''))
            ...
            # Twitch
            twitch_username = st.text_input(
                "Usuário da Twitch",
                value=st.session_state.user_data['esports_profiles'].get(
                    'twitch_username', ''))

            # Steam
            steam_profile = st.text_input(
                "URL do Perfil Steam",
                value=st.session_state.user_data['esports_profiles'].get(
                    'steam_profile', ''))

            # Outras plataformas de jogos
            other_platforms = st.text_area(
                "Outras Plataformas de Jogos (Plataforma: Usuário)",
                value=st.session_state.user_data['esports_profiles'].get(
                    'other_platforms', ''))
            ...
            if submitted:
                ...
                if any(social_media_data.values()):
                    ...
                    with st.spinner("Analisando perfis de redes sociais..."):
                        social_media_info = extract_social_media_info(
                            social_media_data)
                        st.session_state.user_data['social_media'][
                            'analysis'] = social_media_info
                        st.success(
                            "Perfis de redes sociais analisados com sucesso!")
                else:
                    st.warning(
                        "Por favor, forneça pelo menos um perfil de rede social."
                    )
                ...
                if any(esports_profiles_data.values()):
                    ...
                    with st.spinner("Analisando perfis de esports..."):
                        esports_relevance = analyze_social_relevance(
                            esports_profiles_data,
                            st.session_state.user_data['interests'])
                        st.session_state.user_data['esports_profiles'][
                            'relevance'] = esports_relevance
                        st.success("Perfis de esports analisados com sucesso!")
                else:
                    st.warning(
                        "Por favor, forneça pelo menos um perfil de esports.")
    ...
    with col2:
        ...
        st.image(
            ...
            caption="Fãs de esports celebrando em um evento",
    ...
    if 'analysis' in st.session_state.user_data['social_media']:
        ...
        with col1:
            st.metric("Posts Relacionados a Esports",
                      analysis.get('esports_posts', 'N/A'))
        with col2:
            st.metric("Menções a Times", analysis.get('team_mentions', 'N/A'))
        with col3:
            st.metric("Pontuação de Engajamento",
                      analysis.get('engagement_score', 'N/A'))
    ...
    if 'relevance' in st.session_state.user_data['esports_profiles']:
        ...
        col1, col2 = st.columns(2)
        with col1:
            st.metric("Pontuação de Relevância",
                      f"{relevance.get('relevance_score', 0)}/10")
        with col2:
            st.metric("Nível de Confiança",
                      relevance.get('confidence', 'Médio'))
...
# Etapa 5: Painel
    ...
    if st.session_state.user_data['personal'].get('name'):
        st.subheader(
            f"Bem-vindo, {st.session_state.user_data['personal']['name']}!")
        ...
        with col1:
            ...
            st.image(
                "https://images.unsplash.com/photo-1598550457678-aa60413d7c80",
                use_column_width=True)
            ...
            if 'personal' in st.session_state.user_data:
                ...
                st.markdown(
                    f"**Email:** {personal.get('email', 'Não fornecido')}")
                st.markdown(
                    f"**Localização:** {personal.get('city', '')} {', ' + personal.get('state', '') if personal.get('state') else ''}"
                )
        ...
        with col2:
            ...
            if 'interests' in st.session_state.user_data and st.session_state.user_data[
                    'interests']:
                # Criar gráfico de interesses
                fig = create_interest_chart(
                    st.session_state.user_data['interests'])
                st.plotly_chart(fig, use_container_width=True)
            else:
                st.info(
                    "Nenhum dado de interesse disponível. Complete a etapa 2 para ver a visualização de seus interesses."
                )
        ...
        with col1:
            if 'analysis' in st.session_state.user_data['social_media']:
                ...
                fig = create_activity_timeline(
                    st.session_state.user_data['social_media']['analysis'])
                st.plotly_chart(fig, use_container_width=True)
            else:
                st.info(
                    "Nenhum dado de redes sociais disponível. Conecte suas contas para ver insights."
                )
        ...
        with col2:
            ...
            if any(
                    social_media.get(platform) for platform in [
                ...
                for platform, username in social_media.items():
                    if platform in [
                        ...
                        st.markdown(
                            f"- **{platform.replace('_username', '').replace('_profile', '').title()}**: {username}"
                        )
            else:
            ...
            if any(
                    esports_profiles.get(platform)
                    for platform in ['twitch_username', 'steam_profile']):
                for platform, username in esports_profiles.items():
                    if platform in ['twitch_username', 'steam_profile'
                                    ] and username:
                        st.markdown(
                            f"- **{platform.replace('_username', '').replace('_profile', '').title()}**: {username}"
                        )
        ...
        recommendations_col1, recommendations_col2, recommendations_col3 = st.columns(
            3)
        ...
        with export_col1:
            if st.button("Exportar Dados do Perfil"):
                ...
                st.download_button(label="Baixar JSON",
                                   data=profile_json,
        ...
        st.warning(
            "Nenhum dado de perfil disponível. Por favor, complete as etapas anteriores."
        )
